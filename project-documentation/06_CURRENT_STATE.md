...
- Production: commit `6d39665` on `main` (5 September 2026, "The edge rewrite: what the work answers, said once, with no public duration"), Vercel deployment `dpl_ZU6oQorQQcgpiD5YHARRFZHLo3Rg` on project `mindmake` (`prj_GqamX3psD0cGpGCDXRu0ljET7zap`). Before it, commit `0704583` (4 September 2026, "What a crawler and a share card are given") at `dpl_5saD1mJed9NpT9oXA6wgGHSUPGnz`; before that the same day, `fd8c07e` ("The wordmark and the mark are vectors") at `dpl_67GND784KVV4iQVSHPA9CpUXwp1g`; before that, `cf2b2d0` ("The curtain was never there") at `dpl_GQ7wCHKhhRK34Nf5EpxJ3bPRahHc`, then Krish's upload of the two brand exports at `dpl_CAPenq7T8BuRzAia2SrXKkY4fHhw`. Before those, commit `12ace5e` (3 September, the entrance and the story's spine) at `dpl_AFyoEwjLzuG3wr93xuXAYFhVay9a`, and before that the 28 August rebuild, merge `75572542094ddd4e702a877b258b9014f37415c1` (pull request #152) at `dpl_HAoncV1RF3hcvcanqo7Yvc4tuAng`.
- Rollback target: `dpl_5saD1mJed9NpT9oXA6wgGHSUPGnz`, the 4 September production deployment before this one. Not `dpl_AFyoEwjLzuG3wr93xuXAYFhVay9a`: that build carries the 3 September curtain defect (the root as a fixed grid for a second after paint, and no curtain), and rolling back to it would ship that again.
...
|---|---|---|---|
| `submit-mindmake-brief` | v16 | off | The company read, plus the day-14 follow-up enqueue. Deployed from the working tree after the 5 September promotion, with the edge-rewrite copy and the wider personal-email list from `064cbe9`; the deployed body was read back byte-identical to the repository |
| `enrich-company` | v36 | on | Declarative synthesis and tailored choices |
| `get-ai-news` | v69 | off | Restored to the repository and extended with `{view:"board"}`, deployed 2 September 2026. No body still returns the previous shape byte for byte |
...
| `send-follow-ups` | v5 | off | The day-14 follow-up, carrying the edge-rewrite's private-read offer line. Cron only |
...
Last measured 5 September 2026, against the built output.

- Tests: **450 across 29 files**, all passing. The thirty-five added on 29
  August hold the nine dead ends: that each one offers a person, that the
  offer posts the right reason, that it asks for nothing the page already
  holds, that it hands over an address rather than a spinner when even it
  fails, and that the copy stays inside the house style. They exist because
  this is precisely the kind of thing that vanishes in a refactor with no gate
  objecting, which one of the canon promises did for a whole commit. The suite
  has kept growing since, one dated addition at a time; what each covers is in
  the log below.
...
- Page heights at 1440x900: `/` 6.6 screens, `/ai-brain` 9.2, `/ai-gtm` 6.6, `/case-studies` 5.0.
  At 390x844: 8.1, 11.1, 8.6, 4.9.
  Measured 6 September 2026, after the 5 September edge rewrite moved the
  board earlier on `/ai-gtm` and added two GTM-shaped stories, and replaced
  `/ai-brain`'s fork band and pinned climb with `DoorStories`'s two
  client-proof stories. The pinned-climb trade this line used to describe no
  longer applies: that section is deleted.
...
- Browser gates, run against the built output at 1440px and 390px:
  - **Aliveness** (`npm run qa:alive`): no viewport-height of any page is still.
    It photographs five frames 1600ms apart, spanning 6.4 seconds, and reads
    two statistics: the mean change across the whole viewport, and the mean
    across the busiest twentieth of a percent of pixels, which is about a 25 by
    25 patch. The second exists because the mean cannot see a forty-pixel
    instrument moving hard. Floors are 0.15 and 8, both calibrated from
    readings that fall in two groups with nothing between them. Two frames
    900ms apart used to be the window, and a plate's 9.5-second light sweep
    parks for 4.3 of them, so the same viewport read 0.125 on one run and 1.611
    on the next; the wider window sits outside that park. A window more than
    half footer is skipped, because a footer is chrome. It then makes a
    **second, scrubbed pass**: it samples each page at eight scroll offsets and
    requires elements whose state changes with position in all three thirds of
    the page. The first pass alone photographs a stationary viewport, so it
    cannot see a scrubbed build at all and passed a site whose scroll-led
    motion had never been deployed. Clean at both widths: 30 viewports at 1440,
    33 at 390.
  - **Image density** (`npm run qa:images`): no image renders above its intrinsic
    ...
    which is limited by the footage. SVG is exempt. Clean at 1440px (33 images,
    lowest 1.42x) and 390px (33 images, lowest 1.91x).
  - **Section rhythm** (`npm run qa:rhythm`): no two consecutive sections share a
    ...
    script. Clean across 45 sections on four pages.
  - **Card geometry** (`scripts/qa/card-geometry-check.mjs`): every card in a drum
    ...
    18 cards on `/` and 33 on `/case-studies`, both at 177.3px with rows at
    15/89.3/136.3.
  - **One way in** (`scripts/qa/one-way-in-check.mjs`): a page never shows two
    ...
    version of the check counted it. Clean across 10 page/width pairs.
  ...
  - **Screen matrix** (`npm run qa:screens`): eight sizes from 360x800 to
    1920x1080, no section past 1.35 screens, nothing clipped, nothing
    sideways, no fixed chrome over the primary action. Clean: 8 sizes, 4
    pages.
  - **Dead CSS** (`npm run qa:deadcss`): no declaration a component makes is
    beaten by a `.mm-site` reset. Clean at both widths: 6 pages, 13
    properties.
  - **No JS** (`npm run qa:nojs`): every answer the page renders is in the
    served markup with scripting off, and nothing laid out is clipped away
    inside an unscrollable box. Clean: 5 pages, 50 answers present.
  - **The entrance** (`npm run qa:entrance`): the ground never goes light, the
    page is never replaced after it paints, React hydrates without failing,
    and something is moving when it lands. Clean at both widths on every
    path: page replaced 0x, no held or covered state under reduced motion.
  - **Dialog shape** (`scripts/qa/dialog-shape-check.mjs`): the lead dialog has
    a shape at both widths (measured, not eyeballed: 780x634 panel at 1440,
    390x844 at 390, a sticky header and a five-step rail at both).
  - **Chrome and handoff** (`npm run qa:chrome`, `scripts/qa/handoff-check.mjs`):
    last confirmed clean in the 5 September promotion below; both need a
    live `VITE_SUPABASE_URL` to run and could not be re-verified in an
...
2. **The day-14 follow-up is now live, and the first one can send on 11 September 2026.** `submit-mindmake-brief` v13 was deployed straight after the 28 August promotion, so the enqueue and the privacy notice describing it went live together; the function is at v16 now, carrying the 5 September edge-rewrite copy on top of the same enqueue. The deployed body was verified to carry it. `follow_up_queue` was empty at that moment, so nothing predates the notice.
